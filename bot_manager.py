...
class BotManager:
    ...
    def __init__(self, username, config):
        ...
        self.momentum3_potencial_sinal = {} # <--- NOVO
        ...
        self.momentum3_lock = threading.Lock() # <--- NOVO
    ...
    def start_asset_monitor(self, par):
        ...
        try:
            ...
            spike_vol_avg = self.config.get("SPIKE_VOLUME_PERIODO_MEDIA", 20)
    ...
    def stop_asset_monitor(self, par):
        ...
        with self.spike_volume_lock: self.spike_volume_potencial_sinal.pop(par, None)
        with self.momentum3_lock: self.momentum3_potencial_sinal.pop(par, None) # <--- NOVO
    ...
    def run_websocket_client(self, par):
        ...
        def on_message(ws, message):
            ...
            try:
                ...
                if self.config.get("USAR_ESTRATEGIA_SPIKE_VOLUME", False):
                    strategies.verificar_spike_volume_realtime(self, par, k, df_ref, is_pre_alert_window, is_p3v_rsi_confirm_window)
                
                # --- NOVO: MOMENTUM 3 ---
                if self.config.get("USAR_ESTRATEGIA_MOMENTUM_3", False):
                    strategies.verificar_momentum_3_realtime(self, par, k, df_ref, is_pre_alert_window, is_p3v_rsi_confirm_window)
                ...
                if is_pre_alert_window and not sinal_ativo_presente and not cooldown_presente:
                    ...
                    # Prioridade 3: Momentum 3 (NOVO)
                    if not sinal_pre_alerta:
                        with self.momentum3_lock:
                            if par in self.momentum3_potencial_sinal:
                                sinal_pre_alerta = self.momentum3_potencial_sinal[par]
                                origem_pre_alerta = "MOMENTUM-3"
                ...
                if is_breakout_confirm_window and not sinal_ativo_presente and not cooldown_presente:
                    ...
                    with self.breakout_lock: 
                        sinal_final_armado = self.breakout_potencial_sinal.get(par)
                ...
                # 2. Checar Outros Sinais (Janela de 10s)
                if is_p3v_rsi_confirm_window and not sinal_ativo_presente and not cooldown_presente:
                    ...
                    # 2a. P3V
                    with self.p3v_lock:
                        sinal_final_armado = self.p3v_potencial_sinal.get(par)
                        if sinal_final_armado:
                    ...
                    if not sinal_final_armado:
                        with self.rsi_lock: 
                            sinal_final_armado = self.rsi_potencial_sinal.get(par)
                            if sinal_final_armado:
                                origem_final = "RSI-RT"
                    ...
                    # 2c. Spike Volume
                    if not sinal_final_armado:
                        with self.spike_volume_lock:
                            sinal_final_armado = self.spike_volume_potencial_sinal.get(par)
                            if sinal_final_armado:
                                origem_final = "SPIKE-VOLUME-RT"
                                confianca_final = self.config.get('CONFIANCA_SPIKE_VOLUME', 0.85)
                    
                    # 2d. Momentum 3 (NOVO)
                    if not sinal_final_armado:
                        with self.momentum3_lock:
                            sinal_final_armado = self.momentum3_potencial_sinal.get(par)
                            if sinal_final_armado:
                                origem_final = "MOMENTUM-3"
                                confianca_final = self.config.get('CONFIANCA_MOMENTUM_3', 0.88)
                    ...
                    if sinal_final_armado:
                        ...
                        if self.enviar_sinal(par, sinal_info):
                            # Limpa o sinal enviado
                            if origem_final == "PADRAO-P3V":
                                with self.p3v_lock: self.p3v_potencial_sinal.pop(par, None)
                            elif origem_final == "RSI-RT":
                                with self.rsi_lock: self.rsi_potencial_sinal.pop(par, None)
                            elif origem_final == "SPIKE-VOLUME-RT":
                                with self.spike_volume_lock: self.spike_volume_potencial_sinal.pop(par, None)
                            elif origem_final == "MOMENTUM-3": # <--- NOVO
                                with self.momentum3_lock: self.momentum3_potencial_sinal.pop(par, None)
                            sinal_enviado = True; return
    ...
    def publish_config_update_to_web(self):
        ...
        config_data = {
            ...
            "spike_volume": config.get("USAR_ESTRATEGIA_SPIKE_VOLUME", True),
            "momentum_3": config.get("USAR_ESTRATEGIA_MOMENTUM_3", True), # <--- NOVO
            ...
            "spike_volume_periodo_media": config.get("SPIKE_VOLUME_PERIODO_MEDIA", 20),
            "spike_volume_multiplicador": config.get("SPIKE_VOLUME_MULTIPLICADOR", 1.5),
            "spike_reversao_pavio_ratio": config.get("SPIKE_REVERSAO_PAVIO_RATIO", 0.5),
            "spike_reversao_corpo_ratio": config.get("SPIKE_REVERSAO_CORPO_RATIO", 0.3),
            # --- Momentum 3 ---
            "momentum_3_corpo_forte": config.get("MOMENTUM_3_CORPO_FORTE_RATIO", 0.6),
            "momentum_3_corpo_fraco": config.get("MOMENTUM_3_CORPO_FRACO_RATIO", 0.4),
